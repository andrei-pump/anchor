...
///
#[derive(Clone)]
pub struct Program<'a, 'info, T> {
    info: &'a AccountInfo<'info>,
    _phantom: PhantomData<T>,
}

impl<'a, 'info, T: fmt::Debug> fmt::Debug for Program<'a, 'info, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Program").field("info", &self.info).finish()
    }
}

impl<'a, 'info, T> Program<'a, 'info, T> {
    pub(crate) fn new(info: &'a AccountInfo<'info>) -> Program<'a, 'info, T> {
        Self {
            info,
...
impl<'a, 'info, T: Id> TryFrom<&'a AccountInfo<'info>> for Program<'a, 'info, T> {
    type Error = Error;
...
impl<'a, 'info, B, T: Id> Accounts<'a, 'info, B> for Program<'a, 'info, T> {
    #[inline(never)]
    fn try_accounts(
        _program_id: &Pubkey,
...
impl<'a, 'info, T> ToAccountMetas for Program<'a, 'info, T> {
...
impl<'a, 'info, T> ToAccountInfos<'info> for Program<'a, 'info, T> {
    fn to_account_infos(&self) -> Vec<AccountInfo<'info>> {
        vec![self.info.clone()]
    }
}

impl<'a, 'info, T> AsRef<AccountInfo<'info>> for Program<'a, 'info, T> {
    fn as_ref(&self) -> &AccountInfo<'info> {
        self.info
    }
}

impl<'a, 'info, T> Deref for Program<'a, 'info, T> {
...
impl<'a, 'info, T: AccountDeserialize> AccountsExit<'info> for Program<'a, 'info, T> {}

impl<'a, 'info, T: AccountDeserialize> Key for Program<'a, 'info, T> {
    fn key(&self) -> Pubkey {
        *self.info.key
    }
}
